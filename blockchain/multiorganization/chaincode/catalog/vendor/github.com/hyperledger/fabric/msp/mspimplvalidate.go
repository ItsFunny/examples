...
func (msp *bccspmsp) validateSM2Identity(id *identity) error {
	validationChain, err := msp.getCertificationChainForBCCSPSM2Identity(id)
	if err != nil {
		return errors.WithMessage(err, "could not obtain certification chain")
	}

	err = msp.validateSM2IdentityAgainstChain(id, validationChain)
	if err != nil {
		return errors.WithMessage(err, "could not validate identity against certification chain")
	}
...
func (msp *bccspmsp) validateSM2CAIdentity(id *identity) error {
	if !id.sm2Cert.IsCA {
		return errors.New("Only CA identities can be validated")
	}

	validationChain, err := msp.getSM2UniqueValidationChain(id.sm2Cert, msp.getValidityOptsForSM2Cert(id.sm2Cert))
	if err != nil {
		return errors.WithMessage(err, "could not obtain certification chain")
	}
	if len(validationChain) == 1 {
	...
	return msp.validateSM2IdentityAgainstChain(id, validationChain)
}
...
func (msp *bccspmsp) validateSM2IdentityAgainstChain(id *identity, validationChain []*sm2.Certificate) error {
	return msp.validateSM2CertAgainstChain(id.sm2Cert, validationChain)
}
...
func (msp *bccspmsp) validateSM2CertAgainstChain(cert *sm2.Certificate, validationChain []*sm2.Certificate) error {
	// here we know that the identity is valid; now we have to check whether it has been revoked
...
func (msp *bccspmsp) getValidityOptsForSM2Cert(cert *sm2.Certificate) sm2.VerifyOptions {
	// First copy the opts to override the CurrentTime field
	// in order to make the certificate passing the expiration test
	// independently from the real local current time.
	// This is a temporary workaround for FAB-3678
	...
	tempOpts.DNSName = msp.sm2Opts.DNSName
