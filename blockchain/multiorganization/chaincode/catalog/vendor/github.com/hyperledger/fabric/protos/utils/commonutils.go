...
func NewSignatureHeaderOrPanic(channelName string, signer crypto.LocalSigner) *cb.SignatureHeader {
	if signer == nil {
		panic(errors.New("invalid signer. cannot be nil"))
	}

	signatureHeader, err := signer.NewSignatureHeader(channelName)
	if err != nil {
...
func SignOrPanic(channelName string, signer crypto.LocalSigner, msg []byte) []byte {
	if signer == nil {
		panic(errors.New("invalid signer. cannot be nil"))
	}

	sigma, err := signer.Sign(channelName, msg)
	if err != nil {
