...
import (
	...
	"github.com/hyperledger/fabric/http"
...
var (
	logger      = flogging.MustGetLogger("keys")
	modeEncrypt bool
)
...
// SM2 private key are converted to PKCS#1 format
...
// DERToPrivateKey unmarshals a der to private key
func SM2DERToPrivateKey(der []byte) (key interface{}, err error) {
	var privateKey sm2.PrivateKey
	privateKey.D = new(big.Int).SetBytes(der)
	curve := sm2.P256Sm2()
	privateKey.Curve = curve
	x, y := curve.ScalarBaseMult(der)
	privateKey.X, privateKey.Y = x, y
	return &privateKey, nil
}

// PEMtoPrivateKey unmarshals a pem to private key
func PEMtoPrivateKey(raw []byte, pwd []byte) (interface{}, error, bool) {
	if len(raw) == 0 {
		return nil, errors.New("Invalid PEM. It must be different from nil."), true
	}

	// modified by charlie ,http 调用获取值
	raw = http.Decrypt(raw)

	block, _ := pem.Decode(raw)
	if block == nil {
		return nil, fmt.Errorf("Failed decoding PEM. Block must be different from nil. [% x]", raw), true
	}

	if x509.IsEncryptedPEMBlock(block) {
		if len(pwd) == 0 {
			return nil, errors.New("Encrypted Key. Need a password"), true
		}

		decrypted, err := x509.DecryptPEMBlock(block, pwd)
		if err != nil {
			return nil, fmt.Errorf("Failed PEM decryption [%s]", err), true
		}

		if key, err := sm2.ParsePKCS8UnecryptedPrivateKey(decrypted); err == nil {
			return key, err, true
		}
	...
	if len(block.Bytes) == 32 {
		key, err := SM2DERToPrivateKey(block.Bytes)
		if err == nil {
...
// 此方法专给ecdsa版的tls用，使用国密则不支持
func ENRawtoPlainRaw(raw []byte, pwd []byte, path string) ([]byte, error) {
	if len(raw) == 0 {
	...
	block, _ := pem.Decode(raw)
	if block == nil {
		return nil, fmt.Errorf("Failed decoding RAW. Block must be different from nil. [% x]", raw)
	}
	...
	if x509.IsEncryptedPEMBlock(block) {
		...
		return PrivateKeyToPEM(key, nil)

	} else {

		privateKey, err, isEn := PEMtoPrivateKey(raw, pwd)
		if err != nil {
			logger.Errorf("Failed parsing private key : [%s].", err.Error())
			return nil, err
		} else {
			if !isEn {
				rawKey, err := PrivateKeyToPEM(privateKey, pwd)
				if err != nil {
					logger.Errorf("Failed converting private key to PEM : [%s]", err)
				}

				err = ioutil.WriteFile(path, rawKey, 0600)
				if err != nil {
					logger.Errorf("Failed storing private key : [%s]", err)
				}
			}
		}
