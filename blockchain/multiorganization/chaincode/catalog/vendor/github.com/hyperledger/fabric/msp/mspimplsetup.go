...
func (msp *bccspmsp) getSM2CertifiersIdentifier(certRaw []byte) ([]byte, error) {
	// 1. check that certificate is registered in msp.rootCerts or msp.intermediateCerts
	cert, err := msp.getSM2CertFromPem(certRaw)
	if err != nil {
		return nil, fmt.Errorf("Failed getting certificate for [%v]: [%s]", certRaw, err)
	}
	...
	// Search among root certificates
	for _, v := range msp.rootCerts {
		if v.(*identity).sm2Cert.Equal(cert) {
			found = true
			root = true
			break
		}
	...
	if root {
		...
		chain, err = msp.getSM2ValidationChain(cert, true)
		if err != nil {
			return nil, fmt.Errorf("Failed computing validation chain for [%v]. [%s]", cert, err)
		}
	}
	...
	// 4. compute the hash of the certification path
	certifiersIdentifier, err = msp.getSM2CertificationChainIdentifierFromChain(chain)
	if err != nil {
		return nil, fmt.Errorf("Failed computing Certifiers Identifier for [%v]. [%s]", certRaw, err)
	}
...
func (msp *bccspmsp) setupCrypto(conf *m.FabricMSPConfig) error {
	...
	if msp.cryptoConfig.SignatureHashFamily == "" {
		msp.cryptoConfig.SignatureHashFamily = signatureHashFamily
	...
	if msp.cryptoConfig.IdentityIdentifierHashFunction == "" {
		msp.cryptoConfig.IdentityIdentifierHashFunction = identityIdentifierHashFunction
...
func (msp *bccspmsp) setupSM2Crypto(conf *m.FabricMSPConfig) error {
	msp.cryptoConfig = conf.CryptoConfig
	if msp.cryptoConfig == nil {
		// Move to defaults
		msp.cryptoConfig = &m.FabricCryptoConfig{
...
func (msp *bccspmsp) setupSM2CAs(conf *m.FabricMSPConfig) error {
	// make and fill the set of CA certs - we expect them to be there
	if len(conf.RootCerts) == 0 {
		return errors.New("expected at least one CA certificate")
	}
	...
	// will be recreated using the sanitized certs.
	if msp.sm2Opts == nil {
		msp.sm2Opts = &sm2.VerifyOptions{Roots: sm2.NewCertPool(), Intermediates: sm2.NewCertPool()}
	}

	for _, v := range conf.RootCerts {

		cert, err := msp.getSM2CertFromPem(v)
		if err != nil {
			return err
		}
		msp.sm2Opts.Roots.AddCert(cert)
	}
	for _, v := range conf.IntermediateCerts {
		cert, err := msp.getSM2CertFromPem(v)
		if err != nil {
		...
		msp.sm2Opts.Intermediates.AddCert(cert)
	}
	...
	msp.rootCerts = make([]Identity, len(conf.RootCerts))
	for i, trustedCert := range conf.RootCerts {
		id, _, err := msp.getSM2IdentityFromConf(trustedCert)
		if err != nil {
			return err
		}
	...
	msp.sm2Opts = &sm2.VerifyOptions{Roots: sm2.NewCertPool(), Intermediates: sm2.NewCertPool()}
	for _, id := range msp.rootCerts {
		msp.sm2Opts.Roots.AddCert(id.(*identity).sm2Cert)
	}
	for _, id := range msp.intermediateCerts {
		msp.sm2Opts.Intermediates.AddCert(id.(*identity).sm2Cert)
	}
...
func (msp *bccspmsp) setupSM2Admins(conf *m.FabricMSPConfig) error {
	// make and fill the set of admin certs (if present)
	msp.admins = make([]Identity, len(conf.Admins))
	for i, admCert := range conf.Admins {
		id, _, err := msp.getSM2IdentityFromConf(admCert)
		if err != nil {
			return err
		}
...
func (msp *bccspmsp) setupSM2CRLs(conf *m.FabricMSPConfig) error {
	// setup the CRL (if present)
	msp.CRL = make([]*pkix.CertificateList, len(conf.RevocationList))
	for i, crlbytes := range conf.RevocationList {
...
func (msp *bccspmsp) finalizeSetupSM2CAs() error {
	// ensure that our CAs are properly formed and that they are valid
	for _, id := range append(append([]Identity{}, msp.rootCerts...), msp.intermediateCerts...) {
		if !id.(*identity).sm2Cert.IsCA {
			return errors.Errorf("SM2 CA Certificate did not have the CA attribute, (SN: %x)", id.(*identity).cert.SerialNumber)
		}
		if _, err := getSubjectKeyIdentifierFromSM2Cert(id.(*identity).sm2Cert); err != nil {
			return errors.WithMessage(err, fmt.Sprintf("SM2 CA Certificate problem with Subject Key Identifier extension, (SN: %x)", id.(*identity).cert.SerialNumber))
		}

		if err := msp.validateSM2CAIdentity(id.(*identity)); err != nil {
			return errors.WithMessage(err, fmt.Sprintf("SM2 CA Certificate is not valid, (SN: %s)", id.(*identity).cert.SerialNumber))
		}
	}
	...
	msp.certificationTreeInternalNodesMap = make(map[string]bool)
	for _, id := range append([]Identity{}, msp.intermediateCerts...) {
		chain, err := msp.getSM2UniqueValidationChain(id.(*identity).sm2Cert, msp.getValidityOptsForSM2Cert(id.(*identity).sm2Cert))
		if err != nil {
			return errors.WithMessage(err, fmt.Sprintf("failed getting validation chain, (SN: %s)", id.(*identity).cert.SerialNumber))
		}
...
func (msp *bccspmsp) setupSM2NodeOUs(config *m.FabricMSPConfig) error {
	if config.FabricNodeOus != nil {
		...
		msp.clientOU = &OUIdentifier{OrganizationalUnitIdentifier: config.FabricNodeOus.ClientOuIdentifier.OrganizationalUnitIdentifier}
		if len(config.FabricNodeOus.ClientOuIdentifier.Certificate) != 0 {
			certifiersIdentifier, err := msp.getSM2CertifiersIdentifier(config.FabricNodeOus.ClientOuIdentifier.Certificate)
			if err != nil {
				return err
			}
			msp.clientOU.CertifiersIdentifier = certifiersIdentifier
		...
		msp.peerOU = &OUIdentifier{OrganizationalUnitIdentifier: config.FabricNodeOus.PeerOuIdentifier.OrganizationalUnitIdentifier}
		if len(config.FabricNodeOus.PeerOuIdentifier.Certificate) != 0 {
			certifiersIdentifier, err := msp.getSM2CertifiersIdentifier(config.FabricNodeOus.PeerOuIdentifier.Certificate)
			if err != nil {
				return err
			}
			msp.peerOU.CertifiersIdentifier = certifiersIdentifier
...
func (msp *bccspmsp) setupSM2SigningIdentity(conf *m.FabricMSPConfig) error {
	if conf.SigningIdentity != nil {
		sid, err := msp.getSM2SigningIdentityFromConf(conf.SigningIdentity)
		if err != nil {
			sid, err = msp.getSM2SigningIdentityFromConf(conf.SigningIdentity)
			if err != nil {
				return err
...
func (msp *bccspmsp) setupSM2OUs(conf *m.FabricMSPConfig) error {
	msp.ouIdentifiers = make(map[string][][]byte)
	for _, ou := range conf.OrganizationalUnitIdentifiers {

		certifiersIdentifier, err := msp.getSM2CertifiersIdentifier(ou.Certificate)
		if err != nil {
			return errors.WithMessage(err, fmt.Sprintf("failed getting certificate for [%v]", ou))
		}
...
func (msp *bccspmsp) setupTLSCAs(conf *m.FabricMSPConfig) error {
	...
	for _, cert := range append(append([]*x509.Certificate{}, rootCerts...), intermediateCerts...) {
		...
		if err := msp.validateTLSCAIdentity(cert, opts); err != nil {
			return errors.WithMessage(err, fmt.Sprintf("TLS CA Certificate is not valid, (SN: %s)", cert.SerialNumber))
		}
	}
...
func (msp *bccspmsp) setupV1(conf1 *m.FabricMSPConfig) error {
	var err error
	if msp.useSM2 {
		err = msp.preSetupSM2V1(conf1)
		if err != nil {
			return errors.Wrap(err, "failed to msp.preSetupSM2V1")
		}
	} else {
		err = msp.preSetupV1(conf1)
		if err != nil {
			return errors.Wrap(err, "failed to msp.preSetupV1")
		}
	}
...
func (msp *bccspmsp) preSetupSM2V1(conf *m.FabricMSPConfig) error {
	msp.opts = &x509.VerifyOptions{Roots: x509.NewCertPool(), Intermediates: x509.NewCertPool()}
	msp.sm2Opts = &sm2.VerifyOptions{Roots: sm2.NewCertPool(), Intermediates: sm2.NewCertPool()}

	// setup crypto config
	if err := msp.setupSM2Crypto(conf); err != nil {
		return err
	}

	// Setup CAs
	if err := msp.setupSM2CAs(conf); err != nil {
		return err
	}

	// Setup Admins
	if err := msp.setupSM2Admins(conf); err != nil {
		return err
	}

	// Setup CRLs
	if err := msp.setupSM2CRLs(conf); err != nil {
		return err
	}

	// Finalize setup of the CAs
	if err := msp.finalizeSetupSM2CAs(); err != nil {
		return err
	}

	// setup the signer (if present)
	if err := msp.setupSM2SigningIdentity(conf); err != nil {
		return err
	}
	...
	// setup the OUs
	if err := msp.setupSM2OUs(conf); err != nil {
		return err
	}
...
func (msp *bccspmsp) setupV11(conf *m.FabricMSPConfig) error {
	var err error
	if msp.useSM2 {
		err := msp.preSetupSM2V1(conf)
		if err != nil {
			return err
		}
		// setup NodeOUs
		if err := msp.setupSM2NodeOUs(conf); err != nil {
			return err
		}
	} else {
