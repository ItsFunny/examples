...
func (mgr *Mgr) ProcessAttributeRequestsForSM2Cert(requests []AttributeRequest, attributes []Attribute, cert *sm2.Certificate) error {
	attrs, err := mgr.ProcessAttributeRequests(requests, attributes)
	if err != nil {
		return err
	}
	return mgr.AddAttributesToSM2Cert(attrs, cert)
}
...
func (mgr *Mgr) AddAttributesToSM2Cert(attrs *Attributes, cert *sm2.Certificate) error {
	buf, err := json.Marshal(attrs)
	if err != nil {
		return errors.Wrap(err, "Failed to marshal attributes")
	}
...
func (mgr *Mgr) GetAttributesFromSM2Cert(cert *sm2.Certificate) (*Attributes, error) {
	// Get certificate attributes from the certificate if it exists
	buf, err := getAttributesFromSM2Cert(cert)
	if err != nil {
		return nil, err
	}
	// Unmarshal into attributes object
...
func getAttributesFromSM2Cert(cert *sm2.Certificate) ([]byte, error) {
	for _, ext := range cert.Extensions {
		if isAttrOID(ext.Id) {
			return ext.Value, nil
		}
