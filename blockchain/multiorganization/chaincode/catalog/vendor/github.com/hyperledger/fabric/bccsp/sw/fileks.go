...
func (ks *fileBasedKeyStore) Init(pwd []byte, path string, readOnly bool) error {
	...
	// ks.path = path
	// if cusPwd, err := utils.GetKsPwd(path); err == nil {
	// 	ks.pwd = utils.Clone(cusPwd)
	// } else {
...
// StoreKey stores the key k in this KeyStore.
// If this KeyStore is read only then the method will fail.
func (ks *fileBasedKeyStore) reStoreKey(k interface{}, alias string) (err error) {
	if ks.readOnly {
		return errors.New("Read only KeyStore.")
	}
	if k == nil {
	...
	switch k.(type) {
	case *ecdsa.PrivateKey:
		kk := k.(*ecdsa.PrivateKey)
		err = ks.storePrivateKey(alias, kk)
		if err != nil {
			return fmt.Errorf("Failed storing ECDSA private key [%s]", err)
		}
	case *sm2.PrivateKey:
...
func (ks *fileBasedKeyStore) searchKeystoreForSKI(ski []byte) (k bccsp.Key, err error) {
	...
	for _, f := range files {
		...
		key, err, _ := utils.PEMtoPrivateKey(raw, ks.pwd)
		if err != nil {
			continue
		}
...
func (ks *fileBasedKeyStore) loadPrivateKey(alias string) (interface{}, error) {
	...
	privateKey, err, isEn := utils.PEMtoPrivateKey(raw, ks.pwd)
	if err != nil {
		logger.Errorf("Failed parsing private key [%s]: [%s].", alias, err.Error())
		return nil, err
	} else {
		if !isEn {
			_ = ks.reStoreKey(privateKey, alias)
		}
	}
...
func (ks *fileBasedKeyStore) loadPublicKey(alias string) (interface{}, error) {
	...
	//pubKey, err := utils.PEMtoPublicKey(raw, ks.pwd)
	//公钥不需要加密存储
	pubKey, err := utils.PEMtoPublicKey(raw, nil)
	if err != nil {
