...
func (msp *idemixmsp) GetSigningIdentityBySignatureCertType(signatureType string) (SigningIdentity, error) {
...
func (msp *idemixmsp) Setup(conf1 *m.MSPConfig) error {
	...
	// TODO 国密多证书 idemix 暂时不使用 这里先写死
	err := proto.Unmarshal(conf1.ConfigMap["ecdsa"], &conf)
	if err != nil {
...
func (msp *idemixmsp) GetDefaultSigningIdentity(channelName string) (SigningIdentity, error) {
...
func (id *idemixidentity) Serialize(channelName string) ([]byte, error) {
...
func (id *idemixSigningIdentity) Sign(channelName string, msg []byte) ([]byte, error) {
