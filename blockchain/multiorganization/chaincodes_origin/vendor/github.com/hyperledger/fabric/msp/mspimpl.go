...
func newBccspMsp(version MSPVersion) (*bccspmsp, error) {
...
func (msp *bccspmsp) getSM2CertFromPem(idBytes []byte) (*sm2.Certificate, error) {
	if idBytes == nil {
		return nil, errors.New("getSM2CertFromPem error: nil idBytes")
	}

	// Decode the pem bytes
	pemCert, _ := pem.Decode(idBytes)
	if pemCert == nil {
		return nil, errors.Errorf("getSM2CertFromPem error: could not decode pem bytes [%v]", idBytes)
	}
	...
	if isSM2SignedCert(sm2Cert) == false {
		return nil, errors.New("bccspmsp:getSM2CertFromPem not sm2 cert")
	}
	return sm2Cert, nil
}
...
func (msp *bccspmsp) getSM2IdentityFromConf(idBytes []byte) (Identity, bccsp.Key, error) {
	// get a cert
	cert, err := msp.getSM2CertFromPem(idBytes)
	if err != nil {
		return nil, nil, err
	}

	// get the public key in the right format
	certPubK, err := msp.bccsp.KeyImport(cert, &bccsp.SM2X509PublicKeyImportOpts{Temporary: true})

	mspId, err := newSM2Identity(cert, certPubK, msp)
	if err != nil {
		return nil, nil, err
	}
	return mspId, certPubK, nil
...
func (msp *bccspmsp) getSM2SigningIdentityFromConf(sidInfo *m.SigningIdentityInfo) (SigningIdentity, error) {
	if sidInfo == nil {
		return nil, errors.New("getIdentityFromBytes error: nil sidInfo")
	}

	// Extract the public part of the identity
	idPub, pubKey, err := msp.getSM2IdentityFromConf(sidInfo.PublicSigner)
	if err != nil {
		return nil, err
	}

	//idPub.GetIdentifier().SignatureAlgorithm
	// Find the matching private key in the BCCSP keystore
	privKey, err := msp.bccsp.GetKey(pubKey.SKI())
	// Less Secure: Attempt to import Private Key from KeyInfo, if BCCSP was not able to find the key
	if err != nil {
		mspLogger.Debugf("Could not find SM2 SKI [%s], trying KeyMaterial field: %+v\n", hex.EncodeToString(pubKey.SKI()), err)
		if sidInfo.PrivateSigner == nil || sidInfo.PrivateSigner.KeyMaterial == nil {
			return nil, errors.New("SM2 KeyMaterial not found in SigningIdentityInfo")
		}
	...
	return newSM2SigningIdentity(idPub.(*identity).sm2Cert, idPub.(*identity).pk, peerSigner, msp)
}
...
//	if _, ok, err := isSM2Cert(conf.RootCerts[0]); ok && err == nil {
//		msp.useSM2 = true
//	}
//
//	// setup
...
func isSM2Cert(certBytes []byte) (*sm2.Certificate, bool, error) {
	certificate, err := sm2.ReadCertificateFromMem(certBytes)
	if err != nil {
...
func (msp *bccspmsp) Validate(id Identity) error {
	...
	case *identity:
		if msp.useSM2 {
			return msp.validateSM2Identity(id)
		} else {
...
func (msp *bccspmsp) deserializeIdentityInternal(serializedIdentity []byte) (Identity, error) {
	...
	hashFamily := msp.cryptoConfig.SignatureHashFamily
	if isSm2Cert {
		if hashFamily == "SM3" {
			pub, err := msp.bccsp.KeyImport(sm2Cert, &bccsp.SM2X509PublicKeyImportOpts{Temporary: true})
			if err != nil {
				return nil, errors.WithMessage(err, "failed to import certificate's public key")
			}

			return newSM2Identity(sm2Cert, pub, msp)
		} else {
			b64Identity := base64.StdEncoding.EncodeToString(serializedIdentity)
			return nil, errors.Errorf("bccspmsp failed to deserializeIdentityInternal: base64 sm2Identity[%v] with SignatureHashFamily[%s] != SM3", b64Identity, hashFamily)
		}

	} else {
		if hashFamily == "SHA2" || hashFamily == "SHA256" {
			cert, err := x509.ParseCertificate(bl.Bytes)
			if err != nil {
				return nil, errors.Wrap(err, "parseCertificate failed")
			}
			...
			b64Identity := base64.StdEncoding.EncodeToString(serializedIdentity)
			return nil, errors.Errorf("bccspmsp failed to deserializeIdentityInternal: base64 Identity[%v] with SignatureHashFamily[%s] != SM3", b64Identity, hashFamily)
		}
	}
}
	...
	case m.MSPPrincipal_ROLE:
		...
		case m.MSPRole_ADMIN:
			...
			for _, admincert := range msp.admins {
				if msp.useSM2 {
					if bytes.Equal(id.(*identity).sm2Cert.Raw, admincert.(*identity).sm2Cert.Raw) {
...
func (msp *bccspmsp) getSM2CertificationChain(id Identity) ([]*sm2.Certificate, error) {
	mspLogger.Debugf("MSP %s getting certification chain", msp.name)
	...
	// root of trust this MSP has
	case *identity:
		return msp.getCertificationChainForBCCSPSM2Identity(id)
...
// getCertificationChainForBCCSPIdentity returns the certification chain of the passed bccsp identity within this msp
func (msp *bccspmsp) getCertificationChainForBCCSPSM2Identity(id *identity) ([]*sm2.Certificate, error) {
	if id == nil {
		return nil, errors.New("Invalid bccsp identity. Must be different from nil.")
	}
	...
	return msp.getSM2ValidationChain(id.sm2Cert, false)
}
...
func (msp *bccspmsp) getSM2UniqueValidationChain(cert *sm2.Certificate, opts sm2.VerifyOptions) ([]*sm2.Certificate, error) {
	// ask golang to validate the cert for us based on the options that we've built at setup time
	if msp.sm2Opts == nil {
		return nil, errors.New("the supplied identity has no verify sm2 options")
	}
	validationChains, err := cert.Verify(opts)
	if err != nil {
		return nil, errors.WithMessage(err, "the supplied identity is not valid")
...
func (msp *bccspmsp) getSM2ValidationChain(cert *sm2.Certificate, isIntermediateChain bool) ([]*sm2.Certificate, error) {
	validationChain, err := msp.getSM2UniqueValidationChain(cert, msp.getValidityOptsForSM2Cert(cert))
	if err != nil {
		return nil, errors.WithMessage(err, "failed getting validation chain")
	}
...
func (msp *bccspmsp) getSM2CertificationChainIdentifier(id Identity) ([]byte, error) {
	chain, err := msp.getSM2CertificationChain(id)
	if err != nil {
		return nil, errors.WithMessage(err, fmt.Sprintf("failed getting certification chain for [%v]", id))
	}
	...
	// chain[0] is the certificate representing the identity.
	// It will be discarded
	return msp.getSM2CertificationChainIdentifierFromChain(chain[1:])
}
...
func (msp *bccspmsp) getSM2CertificationChainIdentifierFromChain(chain []*sm2.Certificate) ([]byte, error) {
	// Hash the chain
	// Use the hash of the identity's certificate as id in the IdentityIdentifier
	hashOpt, err := bccsp.GetHashOpt(msp.cryptoConfig.IdentityIdentifierHashFunction)
	if err != nil {
...
func (msp *bccspmsp) sanitizeSM2Cert(cert *sm2.Certificate) (*sm2.Certificate, error) {
	// TODO 序列化后报错
	//if isSM2SignedCert(cert) {
	//	// Lookup for a parent certificate to perform the sanitization
	//	var parentCert *sm2.Certificate
	//	chain, err := msp.getSM2UniqueValidationChain(cert, msp.getValidityOptsForSM2Cert(cert))
	//	if err != nil {
	//		return nil, err
	//	}
	//
