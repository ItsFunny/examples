...
func isSM2SignedCert(cert *sm2.Certificate) bool {
	return cert.SignatureAlgorithm == sm2.SM2WithSM3 ||
		cert.SignatureAlgorithm == sm2.SM2WithSHA1 ||
		cert.SignatureAlgorithm == sm2.SM2WithSHA256
}
...
func sanitizeSM2SignedCert(cert *sm2.Certificate, parentCert *sm2.Certificate) (*sm2.Certificate, error) {
	...
	expectedSig, err := utils.SM2SignatureToLowS(parentCert.PublicKey.(*ecdsa.PublicKey), cert.Signature)
	if err != nil {
		return nil, err
	}
	...
	//    encoding
	var newCert certificate
	newCert, err = certFromSM2X509Cert(cert)
	if err != nil {
		return nil, err
	}
...
func certFromSM2X509Cert(cert *sm2.Certificate) (certificate, error) {
	var newCert certificate
	_, err := asn1.Unmarshal(cert.Raw, &newCert)
	if err != nil {
		return certificate{}, errors.Wrap(err, "unmarshalling of the certificate failed")
...
func sm2CertToPEM(certificate *sm2.Certificate) string {
	cert, err := certFromSM2X509Cert(certificate)
	if err != nil {
		mspIdentityLogger.Warning("Failed converting certificate to asn1", err)
		return ""
	}
