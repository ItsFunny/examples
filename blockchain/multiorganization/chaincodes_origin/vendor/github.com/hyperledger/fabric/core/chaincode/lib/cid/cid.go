...
func GetSM2X509Certificate(stub ChaincodeStubInterface) (*sm2.Certificate, error) {
	c, err := New(stub)
	if err != nil {
		return nil, err
	}
	return c.GetSM2X509Certificate()
}
...
func (c *clientIdentityImpl) GetSM2X509Certificate() (*sm2.Certificate, error) {
	return c.sm2Cert, nil
}
...
func (c *clientIdentityImpl) init() error {
	...
	// 先判断是否是sm2证书
	sm2Cert, isSm2Cert, err := utils.IsSM2Cert(block.Bytes)
	if err != nil {
		return errors.Wrapf(err, "failed to parse certificate")
	}
	if isSm2Cert {
		c.sm2Cert = sm2Cert
		attrs, err := attrmgr.New().GetAttributesFromSM2Cert(sm2Cert)
		if err != nil {
