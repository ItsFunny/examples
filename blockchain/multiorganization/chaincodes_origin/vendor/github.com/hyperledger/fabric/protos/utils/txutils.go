...
func CreateSignedEnvelopeWithTLSBinding(txType common.HeaderType, channelID string, signer crypto.LocalSigner, dataMsg proto.Message, msgVersion int32, epoch uint64, tlsCertHash []byte) (*common.Envelope, error) {
	...
	if signer != nil {
		payloadSignatureHeader, err = signer.NewSignatureHeader(channelID)
		if err != nil {
			return nil, err
		}
	}
	...
	var sig []byte
	if signer != nil {
		sig, err = signer.Sign(channelID, paylBytes)
		if err != nil {
			return nil, err
		}
	}
...
func CreateSignedTx(channelId string, proposal *peer.Proposal, signer msp.SigningIdentity, resps ...*peer.ProposalResponse) (*common.Envelope, error) {
	if len(resps) == 0 {
	...
	signerBytes, err := signer.Serialize(channelId)
	if err != nil {
		return nil, err
	}
...
func CreateProposalResponse(channelId string, hdrbytes []byte, payl []byte, response *peer.Response, results []byte, events []byte, ccid *peer.ChaincodeID, visibility []byte, signingEndorser msp.SigningIdentity) (*peer.ProposalResponse, error) {
	...
	endorser, err := signingEndorser.Serialize(channelId)
	if err != nil {
	...
	signature, err := signingEndorser.Sign(channelId, append(prpBytes, endorser...))
	if err != nil {
...
// signing identity
func GetSignedProposal(channelId string, prop *peer.Proposal, signer msp.SigningIdentity) (*peer.SignedProposal, error) {
	...
	signature, err := signer.Sign(channelId, propBytes)
	if err != nil {
		return nil, err
	}
...
func MockSignedEndorserProposal2OrPanic(chainID string, cs *peer.ChaincodeSpec, signer msp.SigningIdentity) (*peer.SignedProposal, *peer.Proposal) {
	serializedSigner, err := signer.Serialize(chainID)
	if err != nil {
		panic(err)
	}
	...
	sProp, err := GetSignedProposal(chainID, prop, signer)
	if err != nil {
		panic(err)
	}
