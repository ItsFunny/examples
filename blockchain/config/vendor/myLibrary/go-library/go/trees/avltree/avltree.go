...
func (t *Tree) Put(comparator utils.Comparator,key interface{}, value interface{}) {
	t.put(comparator,key, value, nil, &t.Root)
}

// Get searches the node in the tree by key and returns its value or nil if key is not found in tree.
// Second return parameter is true if key was found, otherwise false.
// Key should adhere to the comparator's type assertion, otherwise method panics.
func (t *Tree) Get(comparator utils.Comparator,key interface{}) (value interface{}, found bool) {
	n := t.Root
	for n != nil {
...
// Key should adhere to the comparator's type assertion, otherwise method panics.
func (t *Tree) Remove(comparator utils.Comparator,key interface{}) {
	t.remove(comparator,key, &t.Root)
}
...
//
// Key should adhere to the comparator's type assertion, otherwise method panics.
func (t *Tree) Floor(comparator utils.Comparator,key interface{}) (floor *Node, found bool) {
	found = false
	n := t.Root
	for n != nil {
		c := comparator(key, n.Key)
		switch {
		case c == 0:
			return n, true
		case c < 0:
...
//
// Key should adhere to the comparator's type assertion, otherwise method panics.
func (t *Tree) Ceiling(comparator utils.Comparator,key interface{}) (floor *Node, found bool) {
	found = false
	n := t.Root
	for n != nil {
		c := comparator(key, n.Key)
...
func (t *Tree) put(comparator utils.Comparator,key interface{}, value interface{}, p *Node, qp **Node) bool {
	q := *qp
	if q == nil {
		t.Size++
	...
	a := (c + 1) / 2
	var fix bool
	fix = t.put(comparator,key, value, q, &q.Children[a])
	if fix {
...
func (t *Tree) remove(comparator utils.Comparator,key interface{}, qp **Node) bool {
	q := *qp
	if q == nil {
		return false
	}
	...
	a := (c + 1) / 2
	fix := t.remove(comparator,key, &q.Children[a])
	if fix {
