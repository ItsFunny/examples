...
do
    ...
    peer chaincode instantiate -o orderer.demo.com:5050 --tls true --cafile ${ORDERER_CA} -C ${channelName} -n democc -v 1.0 -c '{"Args":["init","a","b","100"]}' -P "OutOf (1,'Org0MSP.peer','Org1MSP.peer','Org2MSP.peer','Org3MSP.peer','Org4MSP.peer','Org5MSP.peer')"
    if [[ $? -ne 0 ]]; then
        echo "实例化链码失败"
        exit -1
    fi
